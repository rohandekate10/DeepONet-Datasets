...
from data.data_functions import DNO_Y_transform, DNO_Y_itransform, Theta_to_U, Theta_to_Z
...
class DeepXDE_DeepONet(object):
    ...
    def __init__(self,Theta, noise, nsteps, Y, net, lr, epochs, N, 
        ...
        u_train = Theta_to_U(noise,Theta,nsteps,coarse,rank).astype(np.float32)
        y_train = Theta_to_Z(Theta,rank).astype(np.float32)
        G_train = DNO_Y_transform(Y).astype(np.float32)
